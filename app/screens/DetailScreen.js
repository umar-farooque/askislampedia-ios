...
function DetailScreen({ route, navigation }) {
  ...
  let handleHeaderButton = () =>
    navigation.navigate("Webview", { url: mainUrl });
  useEffect(() => {
    getArticles.request();
    navigation.setOptions({
      headerRight: (props) => (
        <HeaderReadMore {...props} onPress={handleHeaderButton} />
      ),
    });
  }, []);
  ...
  let handleLinkPress = (evt, href, htmlAttr) => {
    ...
    if (href.includes("askislampedia") && !href.includes(quran)) {
      ...
      navigation.navigate("Webview", { url: href });
      console.log(href);
    } else {
      console.log(href);
    }
